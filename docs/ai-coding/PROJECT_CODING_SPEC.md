...
- 修改公共模板时，必须先修改 `utils` 源头，再同步到 `user`、`rag`、`ai-memory` 等业务微服务本地副本。
- 当前项目不得单独长期修改公共 `Example*` 模板，避免不同微服务的 AI 参考样例漂移。
- 当前项目确实需要 AI 记忆服务专属示例时，新增 `docs/ai-coding/project-examples` 或 `docs/ai-coding/business-examples`，不要放进公共 `examples`。
...
- 需要描述同级仓库时，使用 `../utils`、`../gateway`、`../user`、`../message`、`../report`、`../ai`、`../ai-agent`、`../rag` 这类相对路径，不使用开发者机器上的完整目录。
...
- 路径使用复数名词，例如 `/api/ai-memory/memories`、`/api/ai-memory/candidates`。
- 不使用动词路径表达标准 CRUD，例如 `/save`、`/update`、`/remove`、`/select`、`/page`。
- 查询使用 `GET`，分页通过集合资源表达，例如 `GET /api/ai-memory/memories?patientId=1001&current=1&size=10`。
...
- 修改和删除时，资源 ID 放在 path 里，例如 `PUT /api/ai-memory/memories/{id}`、`DELETE /api/ai-memory/memories/{id}`。
- 写入 body 不重复表达 path 已经表达的动作；body 只承接资源字段和乐观锁 `version`。
- 当前租户优先从请求头、登录上下文或路径父资源中获取；不要在所有 body 中机械重复租户字段。
- 授权、绑定、召回、索引和治理等关系动作按子资源表达，例如 `POST /api/ai-memory/memories/{memoryId}/archive-events` 创建归档事件。
- 无法自然映射为资源 CRUD 的业务动作，也要优先转成资源创建或子资源维护，例如语义索引重建使用 `POST /api/ai-memory/vector-rebuild-tasks` 创建重建任务。
...
- 公开方法和关键业务方法必须有 JavaDoc，说明用途、关键参数、返回值和记忆/租户/患者/权限副作用。
- 方法参数必须说明业务含义；不能只写“参数”或重复变量名。
- 认证授权、租户、患者隔离、权限、MQ、Qdrant 索引、Redis 缓存、数据库乐观锁、SQL 参数校验、事务、异常处理、返回值组装等关键逻辑必须在逻辑块前说明“为什么这样做”和“守住什么边界”。
- 注释要解释业务目的、记忆可见性、风险边界和框架衔接，不要只翻译代码或重复方法名。
...
需要数据权限的记忆业务表必须走 `DataPermissionInterceptor` 的全局表规则，不要在 Service 查询方法里为每个页面手写数据范围过滤。没有负责人或部门归属语义的记忆事件、索引任务等表应放入忽略表或不声明表规则。
